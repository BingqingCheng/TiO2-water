...
paste H2O-TiO2-pos-1.xyz H2O-TiO2-frc-1.xyz | awk -v lx=$lx -v ly=$ly -v lz=$lz 'NF==2{print $1} $1=="Ti" || $1=="O" || $1=="H" {print $1,$2,$3,$4,$6,$7,$8} $1=="i"{printf("Lattice=\"%4.4f 0 0 0 0 %4.4f 0 0 0 0 %4.4f\" energy=%4.8f Properties=species:S:1:pos:R:3:force:R:3\n", lx,ly,lz, $NF)}' > finished/cp2k-$f
